...
- **`core.self_protection.*`** — blocks the agent from editing ptuf's own
  binary, config, plugins, hook script, or any supported agent's own
  settings (hook registration, permission allowlist, MCP servers — e.g.
  `~/.claude/settings.json`, `.codex/config.toml`, `.cursor/hooks.json`).
  The agent cannot turn ptuf off or widen its own permissions mid-session.
