...
pub struct TargetPaths {
    ...
    /// `<hooks_path-parent>/scripts/ptuf-hook-copilot.sh`.
    pub bash_script_path: PathBuf,
    /// `<hooks_path-parent>/scripts/ptuf-hook-copilot.ps1`.
...
fn install_cloud(targets: &TargetPaths, dry_run: bool) -> Result<InstallOutcome, InitError> {
    ...
    let already_present = entry_present
        && script_matches(&targets.bash_script_path, BASH_WRAPPER_BODY)?
        && script_matches(&targets.powershell_script_path, PWSH_WRAPPER_BODY)?;
...
pub(crate) fn is_ptuf_hook_command(cmd: &str) -> bool {
    command_invokes_ptuf_hook(cmd) || command_script_path(cmd).is_some()
}
...
fn has_existing_cloud_hook(root: &Value) -> bool {
    pre_tool_use_commands(root)
        .iter()
        .any(|cmd| command_script_path(cmd).is_some())
}
...
mod tests {
    ...
    #[test]
    fn command_script_path_matches_basename() {
        assert_eq!(
            command_script_path("bash .github/hooks/scripts/ptuf-hook-copilot.sh"),
            Some(".github/hooks/scripts/ptuf-hook-copilot.sh"),
        );
        assert_eq!(
            command_script_path("pwsh -File .github/hooks/scripts/ptuf-hook-copilot.ps1"),
            Some(".github/hooks/scripts/ptuf-hook-copilot.ps1"),
        );
        assert_eq!(
            command_script_path("bash /abs/path/to/ptuf-hook-copilot.sh"),
            Some("/abs/path/to/ptuf-hook-copilot.sh"),
        );
        assert_eq!(command_script_path("ptuf hook copilot"), None);
        assert_eq!(command_script_path("bash some-other-hook.sh"), None);
    }
