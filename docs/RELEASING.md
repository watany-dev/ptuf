...
4. **npm Trusted Publishing for v0.5.0+** — after the first manual bootstrap
   ...
   an `NPM_TOKEN` secret. If a release partially publishes, re-run the
   workflow: already-published npm packages are detected with
   `npm view <name>@<version>` and skipped.
