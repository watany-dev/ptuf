...
| --- | --- | --- | --- | --- |
| [x] | `gap_cmdsubst_outer_nonreader_surfaces_sensitive_token` | `src/rules/sensitive_bash_read.rs` `mod tests` | `HookInput` Bash: `echo $(cat .env)` | `subst_argv` に `cat`+`.env` かつ `assert_ask` (ADR 0008) |
...
| [x] | *(corpus)* `gap-*` → `must_catch` 昇格 | `tests/bypass/corpus.jsonl` | 上記 3 件を直した PR | brace + unicode-homoglyph は `must_catch` (ADR 0007); cmdsubst-outer-nonreader は `must_catch` / ask (ADR 0008) |
