...
| D8 | path 正規化が浅い (`~`/`$HOME` 展開のみ、Bash redirect target と file-tool path が別正規化を経由) | `FilePath` を `PathFact { tool, raw, expanded, absolute, canonical_or_raw, origin }` に拡張 (`pub type FilePath = PathFact;` で API 互換)。`PathOrigin` で `ToolInputDirect` / `ToolInputNested` / `ApplyPatch` / `BashRedirect` を区別し、engine が Bash redirect target も `BashRedirect` 起源の `PathFact` として `ProtectedPaths::classify_input_with_paths` に渡すことで `echo y > .claude/settings.json` を `core.self_protection.claude-settings` で deny。`ProtectedPaths::collect_with_env` は target 側の `canonical_or_raw` を pre-cache し、`path_matches` は candidate 側のみ canonicalize する (`src/facts/path.rs::PathFact`, `src/engine.rs::bash_redirect_path_facts`, `src/self_paths.rs::canonicalise_each`) |
