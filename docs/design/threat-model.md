...
| ID | 脅威 | 影響 | 緩和策 |
|---|---|---|---|
| T-1 | 同 uid プロセスが audit JSONL を append 中に書き込み割込ませて record を破壊 | medium | flock(2) advisory lock で 1 record 単位 atomic append (`src/audit/writer.rs`)。閲覧 CLI は shared lock で length snapshot を取り、読み取り開始時点で完成していた範囲だけを対象にする |
...
| I-4 | audit JSONL の control character が `ptuf audit` の text 出力で terminal を偽装する | medium | `src/cli/run.rs` の text renderer が C0 / DEL / C1 / BiDi を escape し、1 record = 1 line を保つ |
...
| D-5 | 改行無しの巨大 audit 行で viewer が OOM する | medium | 1 行 `MAX_AUDIT_RECORD_BYTES` (1 MiB, `src/audit/read.rs`)。一覧の保持は O(limit) |
