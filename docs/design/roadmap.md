...
- `ptuf hook kiro` (Kiro `preToolUse` payload 正規化、tool 名 alias と
  ...
  reserved rule の流用)
- `ptuf init kiro` (`<repo>/.kiro/agents/*.json` と
  `<global_root>/agents/*.json` を列挙して各 agent に `matcher: "*"` の
  `preToolUse` hook を append、`<global_root>/settings/cli.json` の
  `chat.defaultAgent` を authoritative として effective default agent の
  coverage を検証。`--set-default` / `--workspace-only` / `--global` /
  `--new-agent` で挙動を切替)
