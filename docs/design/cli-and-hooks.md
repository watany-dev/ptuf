...
> bootstrap 段階では `ptuf` バイナリは引数を取らず stdin → exit code の単機能。
> 上記サブコマンドは [`roadmap.md`](roadmap.md) の v0.1〜v0.3 で順次追加する。
...
bootstrap 段階の最小登録は README の例を参照 (引数なしで `ptuf` を起動する形)。
